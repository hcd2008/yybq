...
class yybq {
    ...
    function __construct($moduleid) {
		global $db, $table, $table_n,$table, $MOD;
	...
	function get_one() {
		return $this->db->get_one("SELECT * FROM {$this->table} WHERE id=$this->itemid");
		
	}
	//得到一个营养素
	function get_one_nutrition($id){
		$res=$this->db->get_one("select en_name,name,unit,en_unit from {$this->table_n} where id=$id");
		return $res;
	}
	//得到一个配料的营养素质量
	function get_one_weight($f,$w,$n){
		$res=$this->db->get_one("select $n from {$this->table} where id=$f");
		$zhi=$res[$n];
		$result=$zhi*$w/100;
		return $result;
	}
	function get_nutrition(){
		$res=$this->db->query("select * from {$this->table_n} order by id");
		while($r=$this->db->fetch_array($res)){
			$r['en_name_true']=str_replace('_',' ',$r['en_name']);
			$list[$r['en_name']]=$r;
		}
		return $list;
	}

	function get_list($condition = '1=1', $order = 'id ASC', $cache = '') {
	...
	//用户自定义的配料列表
	function get_list_self($condition = '1=1', $order = 'id ASC', $cache = '') {
		global $MOD, $pages, $page, $pagesize, $offset, $items, $sum;
		if($page > 1 && $sum) {
			$items = $sum;
		} else {
	...
	//用户的自定义配料
	function get_mylist($condition = '1=1', $order = 'id ASC', $cache = '') {
		global $MOD, $pages, $page, $pagesize, $offset, $items, $sum;
		if($page > 1 && $sum) {
			$items = $sum;
		} else {
