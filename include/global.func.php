...
//分类列表 只到2级
function category_list($moduleid=23,$catid=0,$title='--请选择分类--',$name='catid'){
	global $db;
	$str="<select name='".$name."'><option value=0>".$title."</option>";
	$res=$db->query("select catid,catname from bq_category where moduleid=$moduleid and parentid=$catid order by catid");
	while($r=$db->fetch_array($res)){
		if($catid==$r['catid']){
			$str.="<option selected value='".$r[catid]."'>".$r[catname]."</option>";
		}else{
			$str.="<option disabled value='".$r[catid]."'>".$r[catname]."</option>";
		}
		
		$pid=$r['catid'];
		$son=$db->query("select catid,catname from bq_category where moduleid=$moduleid and parentid=$pid order by catid");
		while($t=$db->fetch_array($son)){
			$str.="<option value='".$t[catid]."'>&nbsp;&nbsp;--".$t[catname]."</option>";
		}
	}
	$str.="</select>";
	return $str;
